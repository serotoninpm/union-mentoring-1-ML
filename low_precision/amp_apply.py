...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=16, pin_memory=False, drop_last=True)

testset = torchvision.datasets.CIFAR10(root='./datasets', train=False, download=True, transform=transform_test)
testloader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=False)
...
#json파일로 저장
file_path = "./result/" + 'amp_num_worker_16_pin_False.json'
data = {}
data['result'] = []
data['result'].append({
